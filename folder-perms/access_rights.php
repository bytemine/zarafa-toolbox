...
if(count($argv) != 3) {
    print "Usage: " . $argv[0] . " <user/group> dry/nodry [user]\n";
    print "\n";
    print "If the optional user is given, the script will only work on this\n";
	print "specific store. Otherwise all users are operated on.\n";
    exit(1);
}
...
function set_perms_for_user($store, $session, $user)
{
	...
	//
	// Full write
	$perm_writeveryone = array("userid" => $id, "type" => 2, "rights" => 1531, "state" => 1);
	$perm_writeforgroup = array("userid" => $id, "type" => 2, "rights" => 1275, "state" => 1);
	...
	if($dry == "nodry") {
		...
		printf("dry run, not setting permissions for user '%s'\n", $user);
	}
}
...
if(!isset($argv[4])) {
	...
	while($row = mysql_fetch_array($result)) {
		...
		if(! in_array(strtolower($user), array_map('strtolower',$blacklisted_users)) ) {
			set_perms_on_user($store, $session, $user);	
		} else {
			printf("User '%s' is blacklisted.\n", $user);
		}

	}
} else {
	set_perms_on_user($store, $session, $argv[4]));
}	
